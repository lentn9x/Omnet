//
// Generated file, do not edit! Created by nedtool 4.6 from CoveragePkt.msg.
//

#ifndef _COVERAGEPKT_M_H_
#define _COVERAGEPKT_M_H_

#include <omnetpp.h>

// nedtool version check
#define MSGC_VERSION 0x0406
#if (MSGC_VERSION!=OMNETPP_VERSION)
#    error Version mismatch! Probably this file was generated by an earlier version of nedtool: 'make clean' should help.
#endif



// cplusplus {{
#include "NetwPkt_m.h"
#include "GeoNode.h"
// }}

/**
 * Class generated from <tt>CoveragePkt.msg:10</tt> by nedtool.
 * <pre>
 * message CoveragePkt extends NetwPkt
 * {
 *     GeoNode destNode;
 *     GeoNode interNode;
 *     GeoNode baseNode;
 *     string route[];
 *     GeoNode router[];
 *     bool tracker[100];
 *     bool repeat[100];
 *     double length = 0;
 *     double baseLineFactor[3];
 * }
 * </pre>
 */
class CoveragePkt : public ::NetwPkt
{
  protected:
    GeoNode destNode_var;
    GeoNode interNode_var;
    GeoNode baseNode_var;
    opp_string *route_var; // array ptr
    unsigned int route_arraysize;
    GeoNode *router_var; // array ptr
    unsigned int router_arraysize;
    bool tracker_var[100];
    bool repeat_var[100];
    double length_var;
    double baseLineFactor_var[3];

  private:
    void copy(const CoveragePkt& other);

  protected:
    // protected and unimplemented operator==(), to prevent accidental usage
    bool operator==(const CoveragePkt&);

  public:
    CoveragePkt(const char *name=NULL, int kind=0);
    CoveragePkt(const CoveragePkt& other);
    virtual ~CoveragePkt();
    CoveragePkt& operator=(const CoveragePkt& other);
    virtual CoveragePkt *dup() const {return new CoveragePkt(*this);}
    virtual void parsimPack(cCommBuffer *b);
    virtual void parsimUnpack(cCommBuffer *b);

    // field getter/setter methods
    virtual GeoNode& getDestNode();
    virtual const GeoNode& getDestNode() const {return const_cast<CoveragePkt*>(this)->getDestNode();}
    virtual void setDestNode(const GeoNode& destNode);
    virtual GeoNode& getInterNode();
    virtual const GeoNode& getInterNode() const {return const_cast<CoveragePkt*>(this)->getInterNode();}
    virtual void setInterNode(const GeoNode& interNode);
    virtual GeoNode& getBaseNode();
    virtual const GeoNode& getBaseNode() const {return const_cast<CoveragePkt*>(this)->getBaseNode();}
    virtual void setBaseNode(const GeoNode& baseNode);
    virtual void setRouteArraySize(unsigned int size);
    virtual unsigned int getRouteArraySize() const;
    virtual const char * getRoute(unsigned int k) const;
    virtual void setRoute(unsigned int k, const char * route);
    virtual void setRouterArraySize(unsigned int size);
    virtual unsigned int getRouterArraySize() const;
    virtual GeoNode& getRouter(unsigned int k);
    virtual const GeoNode& getRouter(unsigned int k) const {return const_cast<CoveragePkt*>(this)->getRouter(k);}
    virtual void setRouter(unsigned int k, const GeoNode& router);
    virtual unsigned int getTrackerArraySize() const;
    virtual bool getTracker(unsigned int k) const;
    virtual void setTracker(unsigned int k, bool tracker);
    virtual unsigned int getRepeatArraySize() const;
    virtual bool getRepeat(unsigned int k) const;
    virtual void setRepeat(unsigned int k, bool repeat);
    virtual double getLength() const;
    virtual void setLength(double length);
    virtual unsigned int getBaseLineFactorArraySize() const;
    virtual double getBaseLineFactor(unsigned int k) const;
    virtual void setBaseLineFactor(unsigned int k, double baseLineFactor);
};

inline void doPacking(cCommBuffer *b, CoveragePkt& obj) {obj.parsimPack(b);}
inline void doUnpacking(cCommBuffer *b, CoveragePkt& obj) {obj.parsimUnpack(b);}


#endif // ifndef _COVERAGEPKT_M_H_

